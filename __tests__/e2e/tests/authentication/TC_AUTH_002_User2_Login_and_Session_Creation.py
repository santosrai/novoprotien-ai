"""
Test: TC_AUTH_002 - User2 Login and Session Creation

Description:
    Verify User2 (user2@gmail.com / test12345) can successfully log in 
...
async def run_test():
    """Execute TC_AUTH_002 test."""
    config_loader = get_config_loader()
    user = config_loader.get_user("user2")
    
    if not user:
        raise ValueError("Test user 'user2' not found in configuration")
    
    test = BaseTest(
        test_id="TC_AUTH_002",
        test_name="User2 Login and Session Creation",
        environment="local"
    )
